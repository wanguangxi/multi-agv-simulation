...
int main(int argc, char** argv){
  ros::init(argc, argv, "wan_tf_listener");

  if (argc != 3){ROS_ERROR("need slave name as argument"); return -1;};
  robot_name = argv[1];
  dis = std::stod(argv[2]);
  ...
  slave_vel =
    node.advertise<geometry_msgs::Twist>(robot_name+"/cmd_vel", 10);
  ...
  while (ros::ok()){
    ...
    try{
      listener.lookupTransform(robot_name, "/agv0",
    ...
    try{
      listener.lookupTransform("/agv0", robot_name,
    ...
    vel_msg.angular.z = 4.0 * atan2(transformSM.getOrigin().y(),
                                   transformSM.getOrigin().x());
    vel_msg.linear.x = dis * sqrt(pow(transformSM.getOrigin().x(), 2) +
