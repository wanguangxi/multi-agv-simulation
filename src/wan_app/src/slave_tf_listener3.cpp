...
int main(int argc, char** argv){
  ...
  slave_vel =
    node.advertise<geometry_msgs::Twist>("/agv3/cmd_vel", 10);
  ros::Subscriber laser = node.subscribe("/agv3/scan", 1, laserCallBack);
  ...
  while (node.ok()){
    ...
    try{
      listener.lookupTransform("/agv3", "/agv1",
    ...
    try{
      listener.lookupTransform("/agv1", "/agv3",
